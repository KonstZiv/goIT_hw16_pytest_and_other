"""Модуль классов для консольного приложения "персональный помощник"
"""
...
class NothingFound(Exception):
    """создает исключение которое используется как сигнал в случае, если ничего не найдено
    Перехватывается в обработчике ошибок"""
...
class Phone:
    ...
    @property
    def phone(self):
        """определяет проверку атрибута перед установкой значения"""
        return self.__phone
